...
class Example(object):
    def __init__(self, context, utterances, relations, question, qid, ori_start_pos=None,
                 ori_end_pos=None, answer=None, key_uid=None, key_utterance=None, utterances_cased=None):
...
class InputFeature(object):
    def __init__(self, qid, input_ids, token_type_ids,
                 ...
                 start_pos=None, end_pos=None, key_utterance=None):
        self.qid = qid
...
class Dataset(data.Dataset):
    def __init__(self, features, graphs=None, add_self_loop=True):
    ...
    def __getitem__(self, index):
        ...
        if args.use_all_context:
            data_info['key_utterance'] = torch.tensor(1).eq(1)
        else:
            if len(self.features[index].key_utterance) == 0:
                data_info['key_utterance'] = torch.tensor(0).eq(1)
            else:
                data_info['key_utterance'] = torch.tensor(1).eq(1)
        ...
        if self.graphs is not None:
            ...
            if 'utter_mask' in graph and args.add_utter_mask:
                utter_mask = graph['utter_spk_mask']
            else:
                utter_mask = None
            if 'speaker_position_mask' in graph:
                speaker_position_mask = graph['speaker_position_mask']
                data_info['speaker_position_mask'] = torch.tensor(speaker_position_mask, dtype=torch.float)
            ...
            if not args.add_utter_mask:
                adj = torch.cat([adj[0:num_words+num_speaker, :], adj[num_words+num_speaker+num_utter:, :]], dim=0)
                adj = torch.cat([adj[:, 0:num_words+num_speaker], adj[:, num_words+num_speaker+num_utter:]], dim=1)
                node_type = graph['node_type_no_utter']
            else:
                node_type = graph['node_scene_type']
...
def get_dataset(input_file, cache_path, tokenizer, max_length, training=True, key_utterance_path=None, graph_path=None):
    ...
    example_path = os.path.join(cache_path, "example_{}_speaker.pkl".format(postfix))
    ...
    feature_path = os.path.join(cache_path, "feature_{}_speaker_1.pkl".format(postfix))
    ...
    dataset = Dataset(features, graph_data)
    return dataset
...
def read_examples(input_file, tokenizer, training=True, key_utterance_list=None):
    ...
    for scene in tqdm(input_data):
        ...
        for dialogue in paragraphs:
            relations = None
            utterances = list(dialogue['utterances:'])
            utterances_cased = list(dialogue['utterances:'])
                    ...
                    for context_dict in context_dict_list:
                        if key_utterance_list is not None:
                            ...
                            exp = Example(context_dict['context'], context_dict['utterances'], relations,
                                          ...
                                          utterances_cased=context_dict['utterances_cased'])
                        else:
                            exp = Example(context_dict['context'], context_dict['utterances'], relations,
                                          question, qid + '-' + str(context_dict['start_uid']),
                                          utterances_cased=context_dict['utterances_cased'])
                ...
                for answer in qa['answers']:  # during training
                    ...
                    if key_utterance_list is not None:
                        ...
                        exp = Example(context_dict['context'], real_utterances, relations, question, qid,
                                      ...
                                      key_utterance=range_picked_key, utterances_cased=context_dict['utterances_cased'])
                    else:
                        exp = Example(context_dict['context'], real_utterances, relations, question, qid,
                                      ori_start_pos, ori_end_pos, ans_text, key_uid=real_answer_utter_id,
                                      utterances_cased=context_dict['utterances_cased'])
