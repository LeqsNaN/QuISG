...
class MRCModelSpeaker(pretrained_model_class):
    def __init__(self, config):
        ...
        for i in range(self.trm_layers):
            self.transformer_layer.append(BertLayer(config))
    ...
    def forward(
        ...
        p_mask = p_mask.cuda()
        speaker_position_mask = speaker_position_mask.cuda()
        speaker_range_index = speaker_range_index.cuda()
        transformer = getattr(self, self.transformer_name)
        transformer_outputs = transformer(
            input_ids,
            attention_mask=attention_mask,
        ...
        hsd = hidden_states.shape[2]
        speaker_range_index = speaker_range_index.unsqueeze(2).expand(bsz, slen, hsd)
        speaker_embeddings = torch.gather(speaker_embs, -2, speaker_range_index)
        hidden_states = hidden_states + speaker_embeddings

        expanded_attention_mask = transformer.get_extended_attention_mask(attention_mask, input_ids.size(), input_ids.device)

        for i in range(self.trm_layers):
            hidden_states = self.transformer_layer[i](hidden_states=hidden_states,
                                                      attention_mask=expanded_attention_mask)[0]
...
class MRCModelSpeaker1(pretrained_model_class):
    def __init__(self, config):
        super().__init__(config)
        self.start_n_top = config.start_n_top
        self.end_n_top = config.end_n_top
        ...
        self.encoder = self.electra.encoder.layer
        self.trm_layers = args.trm_layers
        self.sigmoid = nn.Sigmoid()
        self.start_predictor = PoolerStartLogits(config)
        self.end_predictor = PoolerEndLogits(config)
    ...
    def forward(
        ...
        embedding_out = self.embeddings(input_ids, token_type_ids=token_type_ids)

        expanded_attention_mask = self.electra.get_extended_attention_mask(attention_mask, input_ids.size(), input_ids.device)

        for i in range(self.trm_layers):
            embedding_out = self.encoder[i](embedding_out, attention_mask=expanded_attention_mask)[0]

        # (bsz, seqlen, hsz)

        speaker_embs = embedding_out.unsqueeze(1) * speaker_position_mask.unsqueeze(3)
        # (bsz, spknum, hsz)
        speaker_embs = speaker_embs.sum(2) / (torch.sum(speaker_position_mask, dim=-1, keepdim=True)+1e-30)

        bsz, slen = speaker_range_index.shape[0], speaker_range_index.shape[1]
        hsd = embedding_out.shape[2]
        speaker_range_index = speaker_range_index.unsqueeze(2).expand(bsz, slen, hsd)
        speaker_embeddings = torch.gather(speaker_embs, -2, speaker_range_index)
        embedding_out = embedding_out + speaker_embeddings

        for i in range(self.trm_layers, len(self.encoder)):
            embedding_out = self.encoder(embedding_out, attention_mask=expanded_attention_mask)[0]
        ...
        start_logits = self.start_predictor(embedding_out, p_mask=p_mask)  # (bsz, seqlen)

        if training:
            end_logits = self.end_predictor(embedding_out, start_positions=start_pos, p_mask=p_mask)
            start_loss = span_loss_fct(start_logits, start_pos)
            end_loss = span_loss_fct(end_logits, end_pos)
            span_loss = (start_loss + end_loss) / 2
            total_loss = span_loss
            ...
            # during inference, compute the end logits based on beam search
            assert context is not None and offset_mapping is not None
            bsz, slen, hsz = embedding_out.size()
            ...
            start_top_index_exp = start_top_index.unsqueeze(-1).expand(-1, -1, hsz)  # shape (bsz, start_n_top, hsz)
            start_states = torch.gather(embedding_out, -2, start_top_index_exp)  # shape (bsz, start_n_top, hsz)
            start_states = start_states.unsqueeze(1).expand(-1, slen, -1, -1)  # shape (bsz, slen, start_n_top, hsz)

            hidden_states_expanded = embedding_out.unsqueeze(2).expand_as(
                start_states
            )  # shape (bsz, slen, start_n_top, hsz)
            p_mask = p_mask.unsqueeze(-1) if p_mask is not None else None
            end_logits = self.end_predictor(hidden_states_expanded, start_states=start_states, p_mask=p_mask)
